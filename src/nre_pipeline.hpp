// preprocessor directive that tells the compiler "only include this file once
// per translation unit" if .h files get included twice (directly or
// transitively) -> redefinition errors preprocessor optimization
#pragma once
...
struct PipelineConfigInfo {
  ...
  // rasterization: polygon mode (fill/wireframe), culling (back/front/none),
  // winding order
  ...
  // which render pass the pipeline is compatible with, attachments(color,
  // depth)
...
public:
  NrePipeline(NreDevice &device, const std::string &vertFilepath,
  ...
  // binds a pipeline to a command buffer, this pipeline will be used for
